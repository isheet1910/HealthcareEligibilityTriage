...
Simulates realistic messy healthcare data

Normalizes payer names using deterministic logic first, then LLM fallback

Applies the five business rules in strict order

Surfaces results in a Streamlit dashboard with three buckets:
...
Write data/appointments_final.csv
...
last_check_history.csv (~175 rows)
...
Member‑ID‑stuffed strings

Missing values

Ambiguous values

High‑turnover payers
...
Code
CLEAN::<payer_code>
These bypass normalization entirely and ensure:

payer_code matches history

member_id matches

last_check_date < 10 days
...
No history OR last check > 30 days

Payer changed

Member ID mismatch

High‑turnover payer + last check > 14 days
...
I used AI coding assistants (Claude Code + Copilot) for:
